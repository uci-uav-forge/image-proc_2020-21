...
from Dataset_Tools.hdri_operators import rotate, add_new_sun, add_rotation_driver, calculate_sun_position
...
class Colour:
	...
	def __str__(self):
		return self._START + ''.join('{:02X}'.format(v) for v in self)
...
parser.add_argument("--alphanumeric", type=str, required=True, help='uppercase letter A-Z or number 0-9')
parser.add_argument("--num_images", type=int, required=True, help='number of images per shape alphanumeric pair')
...
alphanumeric_name = args.alphanumeric
...
alphanumeric_path = os.path.join(os.getcwd(), 'alphanumeric', alphanumeric_name + '.obj')
bpy.ops.import_scene.obj( filepath = shape_path, filter_glob="*.obj;*.mtl" )
bpy.ops.import_scene.obj( filepath = alphanumeric_path, filter_glob="*.obj;*.mtl" )
object_name = shape_name + '_' + alphanumeric_name
dataset_path = os.path.join(os.getcwd(), 'Dataset', object_name)
...
for obj in bpy.context.scene.objects:
	if 'Camera' not in obj.name and 'Light' not in obj.name:
		...
		elif any([True for alphanumeric in alphanumeric_list if alphanumeric in obj.name]):
			obj.name = alphanumeric_name
			alphanumeric = bpy.data.objects[obj.name]
		bpy.data.objects[obj.name].select_set(True)


# target settings
bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY', center='BOUNDS')
alphanumeric.location = (0, 0, .001)
shape.location = (0, 0, 0)
dg = bpy.context.evaluated_depsgraph_get() 
alphanumeric.rotation_mode = 'XYZ' 
...
# scale range is scale for alphanumeric to fit shape, lower bound is scale for alphanumeric to fit shape of size feet=1, upper bound is scale for alphanumeric to fit shape of size feet=2
...
# hdri
bpy.ops.preferences.addon_install(filepath=os.path.join(os.getcwd(), 'Dataset_Tools/hdri-sun-aligner-1_5.zip'))
...
with open(os.path.join(annotations_summary_save_path, 'annotations_summary.csv'), 'w+', newline='') as annotations_summary:
	...
	while img_id < num_images:
		...
		new_alphanumeric_scale = (scale_range_dict[shape_name][1] - scale_range_dict[shape_name][0]) * percent_inc + scale_range_dict[shape_name][0]
		meters_to_feet(shape, random_feet, shape_x_dim, shape_y_dim, shape_z_dim)
		alphanumeric.scale = (new_alphanumeric_scale, new_alphanumeric_scale, new_alphanumeric_scale)

		# random color
		base = Colour.random()
		color_1 = str(base)
		color_2 = str(base.contrast())
		alphanumeric_rgb = Colour.hex_to_rgb(color_1)
		shape_rgb = Colour.hex_to_rgb(color_2)
		material_name = get_material_name(shape)
		bpy.data.materials[material_name].node_tree.nodes["Principled BSDF"].inputs[0].default_value = (alphanumeric_rgb[0], alphanumeric_rgb[1], alphanumeric_rgb[2], 1)
		material_name = get_material_name(alphanumeric)
